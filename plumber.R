...
#* @param account Akun Instagram
#* @param caption Caption yang akan ditulis
#* @param time Waktu posting
#* @param post_date Tanggal posting yyyy-mm-dd
#* @param variable Tipe postingan
#* @param usertags_post user yang akan di tag
#* @post /isntagrampred 
instapred <- function(account,caption,time,post_date,variable,usertags_post) {
  
  userinstra <- as.character(account)
  ...
  df_instagram <- instagram(account)
  ...
  if( json_awal[1]=="error" | json_awal[1]=="warning") { 
  ...
  else if(is.na(as.numeric(time))==TRUE | is.na(ymd(post_date))==TRUE ) {
    ...
    suppressMessages(use_python(py_config()$python))
    
    suppressMessages(install_instaloadeR())
    
    suppressMessages(init_instaloadeR())
    
    options(warn=-1)
    tioantawibawa <- suppressMessages(insta_posts(query = tolower(account), 
                                                  scope = "username",
                                                  max_posts = 100, 
                                                  scrape_comments = F))
    ...
    #######################################api instagram######################
    IV <- suppressMessages(woebin(df_instagram[,c("time","video","jenisupdate","hari","status")],"status"))
    rekomendasijam <- as.character(IV$time[order(IV$time$badprob,decreasing = T),2][1])
    rekomendasijam <- gsub("['['\\)]","",x = rekomendasijam)
    tips1 <- paste0("waktu posting : ",unlist(str_split(rekomendasijam,pattern = ","))[1],
                    " - ",unlist(str_split(rekomendasijam,pattern = ","))[2])
    ...
    #      stopwordku <- read.table("helper/id.stopwords.02.01.2016.txt")
    tambahan <- c(paste0("",account),"ini","untuk","nih","mestinya","lho","klo","kalo","Tks","yg","dpt","and","the","not","you","for","but","suka","with","just","all")
    ...
    ###################implementasi######################################
    #implementation
    imp_caption_txt <- as.vector(caption)
    imp_time_posting <- ifelse(is.na(as.numeric(time)),0,as.numeric(time))
    ...
    imp_date_posting <- post_date
    imp_date_posting <- ymd(imp_date_posting)
    hariini <- as.character(today())
    imp_date_posting <- ifelse(is.na(imp_date_posting)==TRUE,hariini,as.character(imp_date_posting))
    ...
    imp_hashtags <- ifelse(imp_hashtags_count==0,"",paste0(imp_hashtags,"_hashtags"))
    ...
    imp_mentioned <- ifelse(imp_mentioned_count==0,"",paste0(imp_mentioned,"_usermention"))
    ...
    str2 <- paste0("Prediction : ",round(spam_prediction[2],2)*100,"% ; Accuration : ", round(akurasi,2),"%")
    return(paste0(tips1," ; ",tips2," ; ",tips3," ; ",str2))
