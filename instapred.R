#load indonesian's stopword
stopwordku <- read.table("helper/id.stopwords.02.01.2016.txt")


#function to scrap data for dataframe and convert to data frame
instapred <- function(account,caption,time,post_date,variable,usertags_post) {
    ...
    if( json_awal[1]=="error" | json_awal[1]=="warning") { 
    ...
    else if(is.na(as.numeric(time))==TRUE | is.na(ymd(date_posting))==TRUE ) {
      ...
      tioantawibawa <- suppressMessages(insta_posts(query = tolower(account), 
                                   scope = "username",
                                   max_posts = json_awal$graphql$user$edge_owner_to_timeline_media$count, 
                                   scrape_comments = F))
